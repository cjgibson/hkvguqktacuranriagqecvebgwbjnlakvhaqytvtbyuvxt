...
public class LazyPermutationIterator <T extends Object>
     ...
     * Uses '>' for DIRECTION_RIGHT and '<' for DIRECTION_LEFT; if a value is
     * not mobile, it uses '≯' and '≮', respectively. If all permutations have
     * been seen, the direction is presented as '|'.
     * 
     * @param i The location of the value in permutationValue.
     * @return The direction of the value, '<', or '>', or '≯', or '≮', or '|'.
     */
    public char getDirectionChar(int i) {
        if (this.finished) { return '|'; }
        if (this.getDirection(i) > 0) {
            if (this.isMobile(i))
                return '>';
            else
                return '≯';
